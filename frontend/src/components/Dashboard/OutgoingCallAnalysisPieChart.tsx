...
const OutgoingCallAnalysisPieChart: React.FC<OutgoingCallAnalysisPieChartProps> = ({
  className = '',
  title = 'Outgoing Call Analysis',
  height = 300
}) => {
  const chartRef = useRef<HTMLDivElement>(null);
  const chartInstance = useRef<echarts.ECharts | null>(null);
  const [outgoingStats, setOutgoingStats] = useState({
    completed: 0,
    no_answer: 0,
    busy: 0,
    failed: 0
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        setOutgoingStats({
          completed: stats.outgoing_by_status?.completed || 0,
          no_answer: stats.outgoing_by_status?.no_answer || 0,
          busy: stats.outgoing_by_status?.busy || 0,
          failed: stats.outgoing_by_status?.failed || 0
        });
        setError(null);
      } catch (err) {
        console.error('Error fetching outgoing call stats:', err);
        setError('Failed to load call statistics');
        setOutgoingStats({ completed: 0, no_answer: 0, busy: 0, failed: 0 });
      } finally {
        setLoading(false);
      }
    };
  ...
  useEffect(() => {
    ...
    const totalCalls = Object.values(outgoingStats).reduce((sum, count) => sum + count, 0);
  ...
  }, [outgoingStats, title, height, loading]);
