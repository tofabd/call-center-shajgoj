...
class LiveCallsRealtimeService {
  private listeners: Map<string, (update: CallUpdate) => void> = new Map();
  private isListening = false;

  /**
   * Start listening to real-time live call updates
   */
  startListening(): void {
    if (this.isListening) {
      return;
    ...
    if (!echo) {
      console.error('❌ Echo not available - cannot start real-time live calls listening');
      return;
    }

    try {
      echo.channel('live-calls')
        .listen('.call-updated', (update: CallUpdate) => {
          console.log('📞 Real-time live call update:', update);
          this.notifyListeners(update);
        });

      this.isListening = true;
      console.log('✅ Started listening to real-time live call updates');
    } catch (error) {
      console.error('❌ Failed to start real-time live calls listening:', error);
  ...
  stopListening(): void {
    ...
    // Check if Echo is available
    if (!echo) {
      console.warn('⚠️ Echo not available - cannot stop real-time live calls listening');
      this.isListening = false;
      return;
    }

    try {
      echo.leaveChannel('live-calls');
      this.isListening = false;
      console.log('🛑 Stopped listening to real-time live call updates');
    } catch (error) {
      console.error('❌ Failed to stop real-time live calls listening:', error);
      this.isListening = false;
    }
  }
  ...
  /**
   * Subscribe to all live call updates
   */
  ...
  private notifyListeners(update: CallUpdate): void {
    this.listeners.forEach((callback) => {
      try {
        ...
        console.error('Error in live call update callback:', error);
      }
    });
  }
...
export const liveCallsRealtimeService = new LiveCallsRealtimeService();
export default liveCallsRealtimeService;
